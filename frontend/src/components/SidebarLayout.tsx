...
import { 
  ...
  LogOut, Menu, X, Wallet, User, Settings, Users, 
  CalendarClock, Target, HardDriveDownload, PieChart // <-- ALL ICONS IMPORTED HERE
} from "lucide-react";
...
export default function SidebarLayout({ children }: SidebarLayoutProps) {
  ...
  const navigationItems = [
    ...
    { label: "Deep Analytics", path: "/dashboard/analytics", icon: PieChart },
  ...
  return (
    <div className="min-h-screen bg-[#070a13] text-white flex font-sans">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-gray-900/40 backdrop-blur-xl border-r border-white/[0.06] p-6 flex flex-col justify-between
        transform transition-transform duration-300 ease-in-out xl:translate-x-0 xl:static xl:h-screen
      ...
      `}>
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setIsOpen(false)} className="xl:hidden p-1 text-gray-500 hover:text-white">
              <X size={18} />
            </button>
          </div>
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-xs font-mono tracking-wide transition-all ${
                    isActive 
        ...
        <div className="border-t border-white/[0.06] pt-4 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2.5 px-2 mb-2 bg-black/20 p-2 rounded-lg border border-white/[0.02]">
            <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-blue-600 to-emerald-500 p-[1px]">
              <div className="w-full h-full bg-gray-950 rounded-full flex items-center justify-center text-[10px] font-bold text-gray-300">OP</div>
            </div>
            <div className="truncate">
              <p className="text-xs font-medium text-gray-200 truncate">{userName}</p>
              <p className="text-[9px] font-mono text-emerald-400 uppercase tracking-widest">Node Active</p>
            </div>
          </div>
